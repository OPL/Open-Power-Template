 ...
 */
	...
	class Opt_Instruction_Include extends Opt_Compiler_Processor
	{
		...
		public function processNode(Opt_Xml_Node $node)
		{
			...
			{
				$node->addAfter(Opt_Xml_Buffer::TAG_BEFORE, $codeBegin.' if(!'.$view.'->_parse($output, $mode, $cached, false)){ '.$view.'->_template = '.$params['default'].'; '.$view.'->_parse($output, $mode, $cached, true); } '.$codeEnd);
			}
			elseif($node->hasChildren())
			{
				$node->addBefore(Opt_Xml_Buffer::TAG_CONTENT_BEFORE, $codeBegin.' if(!'.$view.'->_parse($output, $mode, $cached, false)){ ');
