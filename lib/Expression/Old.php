...
/**
 * The old expression parser from OPT 2.0. Remove in the final
 * version, it is left just as the code archive.
 *
 * @package Expressions
 */
class Opt_Expression_Old implements Opt_Expression_Interface
{
	// Opcodes
	const OP_VARIABLE = 1;
						...
						if(!is_null($section = $proc->getSection($sectionName)))
						{
							...
							if($id == $count - 1)
							{
								...
								if($saveContext !== null)
								{
									...
									$section['format']->assign('value', $saveContext);
