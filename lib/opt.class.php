	...
	class optClass
	{
		...
		public function fetch($filename, $display = false)
		{
			...
			if($this -> performance)
			{
				...
				{				
					...
					{
						$filename = (!is_null($this->compileId) ? $this -> compileId.'_' : '').optCompileFilenameFull($filename);
				...
				# /OUTPUT_CACHING
					$oldErrorReporting = error_reporting(E_ALL ^ E_NOTICE);
					include($this -> compile.(!is_null($this->compileId) ? $this -> compileId.'_' : '').optCompileFilenameFull($filename));
					error_reporting($oldErrorReporting);
				# OUTPUT_CACHING
				}
				# /OUTPUT_CACHING
		...
		private function doInclude($filename, $default = false)
		{
			...
			if($this -> performance)
			{
				if($default == true)
				{
					if(!file_exists($filename = $this -> compile.(!is_null($this->compileId) ? $this -> compileId.'_' : '').optCompileFilenameFull($filename)))
					{
						return false;
					}
					$oldErrorReporting = error_reporting(E_ALL ^ E_NOTICE);
		...
		{
			$compiled = (!is_null($this->compileId) ? $this -> compileId.'_' : '').optCompileFilename($filename);
			# CUSTOM_RESOURCES
			$resource = 'file';
